...
export class DetailedAnalysis {
  ...
  private initializeContainer(): void {
    ...
    const expandId = `expand-toggle-${Math.random()
      .toString(36)
      .substring(2, 9)}`;
    const detailsId = `analysis-details-${Math.random()
      .toString(36)
      .substring(2, 9)}`;
    ...
    this.container.innerHTML = `
      ...
      <div class="analysis-details ${
        this.options.expandable ? "collapsible" : ""
      }" id="${detailsId}" ${
      this.options.expandable ? 'aria-hidden="true"' : ""
    } tabindex="${this.options.expandable ? "-1" : "0"}">
  ...
  private setupEventListeners(): void {
    ...
    const expandToggle = this.container.querySelector(
      ".expand-toggle"
    ) as HTMLButtonElement;
    const detailsSection = this.container.querySelector(
      ".analysis-details"
    ) as HTMLElement;
  ...
  private updateConfidenceOverview(confidence: number): void {
    const confidenceFill = this.container.querySelector(
      ".confidence-fill"
    ) as HTMLElement;
    const confidenceText = this.container.querySelector(
      ".confidence-text"
    ) as HTMLElement;
    ...
    if (confidenceFill && confidenceText) {
      ...
      const confidenceBar = this.container.querySelector(
        ".confidence-bar"
      ) as HTMLElement;
  ...
  private updateReasoning(reasoning: {
    factual: string[];
    unfactual: string[];
    subjective: string[];
    objective: string[];
  }): void {
    const reasoningText = this.container.querySelector(
      ".reasoning-text"
    ) as HTMLElement;
    ...
    if (reasoningText) {
      ...
      if (
        this.options.expandable &&
        displayText.length > this.options.maxReasoningLength
      ) {
        displayText =
          displayText.substring(0, this.options.maxReasoningLength) + "...";
      }
  ...
  private combineReasoningCategories(reasoning: {
    factual: string[];
    unfactual: string[];
    subjective: string[];
    objective: string[];
  ...
  private updateCategoryBreakdown(
    categories: { factuality: number; objectivity: number },
    confidence: number
  ): void {
  ...
  private updateCategoryItem(
    ...
    const percentageElement = categoryItem.querySelector(
      ".category-percentage"
    ) as HTMLElement;
    const confidenceDots = categoryItem.querySelectorAll(
      ".confidence-dots .dot"
    );
    ...
    if (this.options.showConfidenceIndicators && confidenceDots.length > 0) {
      ...
      const filledDots = Math.round(
        (categoryConfidence / 100) * confidenceDots.length
      );
  ...
  private updateMetadata(analysisResult: AnalysisResult): void {
    const analysisTime = this.container.querySelector(
      ".analysis-time"
    ) as HTMLElement;
    const contentHash = this.container.querySelector(
      ".content-hash"
    ) as HTMLElement;
    ...
    if (contentHash) {
      ...
      contentHash.textContent =
        analysisResult.contentHash.substring(0, 8) + "...";
    }
  }
  ...
  public showErrorState(
    message: string = "Analysis details unavailable"
  ): void {
